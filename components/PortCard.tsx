...
export default function PortCard({ port, onClick, loading }: PortCardProps) {
  const getBackgroundColor = (status: string) => {
    return status === 'Online' ? 'bg-green-100 hover:bg-green-200 dark:bg-green-900 dark:hover:bg-green-800' : 'bg-red-100 hover:bg-red-200 dark:bg-red-900 dark:hover:bg-red-800'
  }
  ...
  return (
    <Card 
      className={`cursor-pointer transition-colors ${getBackgroundColor(port.status)} h-full flex flex-col`}
      onClick={onClick}
    >
      {loading && (
